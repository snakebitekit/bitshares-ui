...
(function(HC) {
    ...
    /*
        ...
        */
    HC.wrap(HC.Series.prototype, "update", function(
        proceed,
        redraw,
        animation
    ) {
    ...
    /*
        *  Remove corresponding indicators for series
        */
    HC.wrap(HC.Series.prototype, "remove", function(
        proceed,
        redraw,
        animation
    ) {
    ...
    /*
        ...
        */
    HC.wrap(HC.Series.prototype, "setData", function(
        proceed,
        redraw,
        animation
    ) {
    ...
    /*
        ...
        */
    HC.wrap(HC.Series.prototype, "addPoint", function(
        proceed,
        options,
        redraw,
        shift,
    ...
    HC.wrap(HC.Series.prototype, "setVisible", function(proceed, vis, redraw) {
        ...
        if (series.indicators) {
            ...
            if (series.chart.legend && series.chart.legend.options.enabled) {
                // #20
    ...
    /*
        ...
        */
    HC.wrap(HC.Point.prototype, "remove", function(
        proceed,
        options,
        redraw,
        animation
    ...
    */
    HC.wrap(HC.Axis.prototype, "render", function(p) {
        ...
        if (this.indicators && !this.hasVisibleSeries) {
            ...
            this.hasData =
                typeof this.hasData == "function"
                    ? function() {
                          return oldHasData;
                      }
                    : oldHasData; // version 4.1.6 vs 4.1.7

            if (
                (typeof this.hasData == "function" && this.hasData()) ||
                (typeof this.hasData !== "function" && this.hasData)
            ) {
                // version 4.1.6 vs 4.1.7
                this.setScale();
            ...
            each(this.series, function(serie, i) {
                if (
                    serie.isVisible ||
                    (serie.isVisible === UNDEFINED && serie.visible)
                ) {
                    hasData = true;
                }
            });
            ...
            this.hasData =
                typeof this.hasData == "function"
                    ...
                    : hasData; // version 4.1.6 vs 4.1.7
    ...
    /*
        ...
        */
    HC.wrap(HC.Tooltip.prototype, "defaultFormatter", function(
        proceed,
        tooltip
    ) {
    ...
    HC.wrap(HC.Tooltip.prototype, "bodyFormatter", function(proceed, items) {
        return HC.map(items, function(item) {
            ...
            return (
                tooltipOptions.pointFormatter || item.point.tooltipFormatter
    ...
    HC.wrap(HC.Point.prototype, "tooltipFormatter", function(
        proceed,
        pointFormat
    ) {
                        ...
                        if (ind.options.tooltip) {
                            ...
                            pointFormat += HC.format(indPointFormat, {
                                point: {
                                    bottomColor:
                                        indLen > 3
                                            ? ind.graph[2].element.attributes
                                                  .stroke.value
                                            : "",
                                    ...
                                    y:
                                        indLen > 3
                                            ? HC.numberFormat(val[2], 3)
                                            : HC.numberFormat(val[1], 3),
                                    color:
                                        indLen > 3
                                            ? ind.graph[1].element.attributes
                                                  .stroke.value
                                            : ind.graph[0].element.attributes
                                                  .stroke.value,
                                    topLine: HC.numberFormat(val[1], 3),
                                    topColor:
                                        ind.graph[0].element.attributes.stroke
                                            .value
                                },
                                series: ind
                            });
                            ...
                            for (k = 0; k < graphLen; k++) {
                                pointFormat +=
                                    ...
                                    ind.graph[k].element.attributes.stroke
                                        .value +
                                    ';">' +
                                    HC.splat(ind.options.names || ind.name)[k] +
                                    "</span>: " +
                                    HC.numberFormat(val[k + 1], 3) +
                                    "<br/>";
                            }
                        }
                    }
    ...
    HC.wrap(HC.Point.prototype, "getLabelConfig", function(
        proceed,
        point,
        mouseEvent
    ) {
    ...
    /*
        ...
        */
    HC.wrap(HC.Legend.prototype, "setItemEvents", function(
        p,
        item,
        legendItem,
        useHTML,
    ...
    Indicator.prototype = {
        ...
        init: function(chart, options) {
            ...
            options.params = merge(
                {},
                Indicator.prototype[options.type].getDefaultOptions(),
                options.params
            );
            ...
            this.name =
                options.name === UNDEFINED ? options.type : options.name;
            this.visible =
        ...
        render: function(redraw) {
            ...
            if (!group) {
                indicator.group = group = renderer
                    .g()
                    .add(chart.indicators.group);
            }
            ...
            if (!series) {
                ...
                arrayValues = Indicator.prototype[options.type].getValues(
                    chart,
                    {points: []},
                    options,
                ...
                this.graph = graph = Indicator.prototype[options.type].getGraph(
                    chart,
                    series,
                    options,
                    this.values
                ...
                if (indicator.options.Axis) {
                    indicator.options.Axis.indicators =
                        indicator.options.Axis.indicators || [];
        ...
        drawGraph: function() {
            ...
            ind.graph = Indicator.prototype[ind.options.type].getGraph(
                ind.chart,
                ind.series,
                ind.options,
                ind.values
        ...
        groupPoints: function(series) {
            ...
            if (series.currentDataGrouping) {
                ...
                groupIntervalFactor =
                    ...
                    1;
                interval =
                    ((series.groupPixelWidth * (xMax - xMin)) /
                ...
                groupedPoints = this.groupData(
                    this.xData.slice(Math.max(0, start - length), end),
                    this.yData.slice(Math.max(0, start - length), end),
                    groupPositions.slice(),
                    this.options.params.approximation
                );
            ...
            this.options.yAxisMin = Math.min.apply(
                null,
                Array.prototype.concat.apply([], this.processedYData)
            ); // new extremes
            this.options.yAxisMax = Math.max.apply(
                null,
                Array.prototype.concat.apply([], this.processedYData)
            ); // new extremes
            ...
            var ind = this,
                ...
                approximationFn =
                    typeof approximation === "function"
                        ? approximation
                        : HC.approximations[approximation],
                i;
                ...
                verticalCenter =
                    ...
                    Math.round(
                        renderer.fontMetrics(
                            legendOptions.itemStyle.fontSize,
                            this.legendItem
                        ).b * 0.3
                    ),
                attr;
        ...
        updateHeightAxes: function(topDiff, add, afterRemove) {
            ...
            if (
                ...
                (!chart.indicators ||
                    ...
                    chart.indicators.allItems.length === 0)
            )
                return;
            ...
            for (; i < len; i++) {
                ...
                if (yAxis.options.id !== "navigator-y-axis") {
                    top =
                        chart.plotTop + indexWithoutNav * (topDiff + newHeight);
    ...
    extend(Axis.prototype, {
        ...
        addAxisPane: function(chart, userOptions) {
            ...
            var topDiff = 20,
                height,
                yLen = chart.options.navigator.enabled
                    ? chart.yAxis.length - 1
                    : chart.yAxis.length, // #17 - don't count navigator
                defaultOptions,
                options;
    ...
    Chart.prototype.callbacks.push(function(chart) {
        ...
        // counter for axes #18
        chart.indicators.haveAxes = 0;
        chart.alignAxes = defined(chart.options.chart.alignAxes)
            ? chart.options.chart.alignAxes
            : true;

        for (i = 0; i < optionsLen; i++) {
            chart.addIndicator(options[i], false);
            if (
                chart.get(options[i].id).data.length - 1 <=
                options[i].params.period
            )
                // SPLAT?
                exportingFlag = false;
        }
